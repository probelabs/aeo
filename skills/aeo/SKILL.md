...
After two completed runs on the same roster, `scripts/change_report.py --baseline <run_N-1> --current <run_N> --brand <Brand>` writes `change.json` and `*-change-report.html` (brand Δpp, prompt transitions, competitor risers/fallers/new/surprises). One side may lack `vendors_judged.json`.
