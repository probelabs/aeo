...
After a full evidence run, `scripts/judge_run.py` does three passes: stance/position/quote on `brand_mentioned` cells, **vendor extract on every completed arm** (hits and misses; config `competitors` are hints, not a ceiling), then a board brief. Then `scripts/render_judge_html.py`. Vendor-only: `python3.11 scripts/judge_run.py --vendors-only <evidence.json|run_dir>`. Do not treat CLI `recommended` as testimony. Brand hit rate stays deterministic `brand_mentioned`. Grok AEO runs must use `GROK_HOME` without MCP and may need `GROK_SANDBOX=workspace` when Docker Desktop makes `docker.sock` a symlink.
