...
After two completed runs on the same roster, `scripts/change_report.py --baseline <run_N-1> --current <run_N> --brand <Brand>` writes `change.json` and `*-change-report.html`: executive narrative (what the Δ means), brand vs field, rank table (↑/↓/NEW/OUT), new/disappeared, risers/fallers, then brand Δpp and prompt transitions. `--floor` (default 1). Ranks and blended rates use engines on both sides. One side may lack `vendors_judged.json`.
