...
After **two** completed boards (same roster), `scripts/change_report.py --baseline <N-1> --current <N> --brand <Brand>` diffs them. Competitor dynamics first: brand vs field, rank table (↑/↓/NEW/OUT), new (known vs surprise), disappeared, risers/fallers — then brand Δpp and prompt miss→hit / hit→miss. `--floor` (default 1) is the NEW/OUT threshold. Ranks use engines on both sides; a skipped engine is labelled, not a market drop. Baseline may lack `vendors_judged.json`. Writes `change.json` + `*-change-report.html`.
