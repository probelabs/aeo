"""python3 -m aeo {init,run,report,validate}"""
...
from aeo.config import Config, load_config, starter_config, write_config
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    if args.prompt:
        prompts = [{"id": args.prompt_id, "text": args.prompt, "intent": None}]
    else:
        if not cfg.prompts:
            print("config has no prompts; pass --prompt", file=sys.stderr)
            return 1
        prompts = [{"id": p.id, "text": p.text, "intent": p.intent} for p in cfg.prompts]
...
def cmd_report(args: argparse.Namespace) -> int:
    ...
    if not files:
        # Fall back to the example fixture so `report` is useful out of the box.
        example = Path(__file__).resolve().parents[2] / "examples" / "xerj" / "aeo-data" / "example-run.json"
        if example.exists():
            files = [example]
        else:
            print(f"no evidence files at {path}", file=sys.stderr)
            return 1
